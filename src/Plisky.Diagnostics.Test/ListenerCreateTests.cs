...
#pragma warning disable SA1600
    ...
    public class ListenerCreateTests {
        ...
        [Theory(DisplayName = nameof(Config_FromString_OverallLevel_Set))]
        [InlineData("[TLV:Off]", SourceLevels.Off)]
        [InlineData("xxxxx", SourceLevels.All)]
        [InlineData("[TLV:Verbose]", SourceLevels.Verbose)]
        [InlineData("[TLV:Information]", SourceLevels.Information)]
        public void Config_FromString_OverallLevel_Set(string initString, SourceLevels expected) {
            var mbc = new MockBilgeConfigure();
            BilgeConfigure bc = mbc;
            bc.Configuration.OverallSourceLevel = System.Diagnostics.SourceLevels.All;
            bc.From.StringValue(initString);

            Assert.Equal(expected, bc.Configuration.OverallSourceLevel);
        }

        [Theory(DisplayName = nameof(ExploratoryX))]
        [InlineData("[TLV:Off][LST:(TEX)]", 2)]
        [InlineData("xxxxx", 0)]
        [InlineData("[LST:(TEX)(TEX)(TEX;a,b,c)]", 1)]
 ...
 () is included, it causes the test filter to fail.
        ...
        [Fact(DisplayName = nameof(Apply_Resolver_EnablesResolver))]
        public void Apply_Resolver_EnablesResolver() {
            var mbc = new MockBilgeConfigure();
            BilgeConfigure bc = mbc;
        ...
        [Theory(DisplayName = nameof(ExploratoryX))]
        [InlineData("[TLV:Off][LST:(TEX)]", 1)]
        [InlineData("xxxxx", 0)]
        [InlineData("[LST:(TEX)(TEX)(TEX;a,b,c)]", 3)]
        public void ExploratoryX(string initString, int handlerStringCount) {
            var mbc = new MockBilgeConfigure();
            BilgeConfigure bc = mbc;
            bc.From.StringValue(initString);

            Assert.Equal(handlerStringCount, bc.Configuration.HandlerStrings.Length);
        }

        [Fact(DisplayName = nameof(ConfigurationResolver_Exploratory))]
        public void ConfigurationResolver_Exploratory1() {
            var mbc = new MockBilgeConfigure();
            BilgeConfigure bc = mbc;

            Assert.True(bc.Configuration.OverallSourceLevel == System.Diagnostics.SourceLevels.Off);
            Assert.True(bc.Configuration.HandlerStrings.Length == 0);
        }

        [Fact(DisplayName = nameof(ConfigurationResolver_Exploratory))]
        public void ConfigurationResolver_Exploratory() {
            var mbc = new MockBilgeConfigure();
            BilgeConfigure bc = mbc;
            bc.From.EnvironmentVariable("Dummy");

            Assert.True(mbc.EnvironmentVariableCallIdx == 0);
            Assert.True(mbc.StaticFileCallIdx == -1);
        }

        [Fact(DisplayName = nameof(ConfigurationResolver_Exploratory))]
        public void ConfigurationResolver_Exploratory2() {
            var mbc = new MockBilgeConfigure();
            BilgeConfigure bc = mbc;
            bc.From.FileSystemWatcher("Dummy");
        }
        ...
        public void RollingFileHandler_NameGeneration_Works(string mask, string expected, int dd, int mm, int yy, int hh, int min, int sec, string pid) {
            ...
            var fn = sut.GetFilenameFromMask("", roro.FileName);
